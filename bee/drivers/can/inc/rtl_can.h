...
#define IS_CAN_INT_CONFIG(CONFIG)                                                                  \
    (((CONFIG) == CAN_RAM_MOVE_DONE_INT) || ((CONFIG) == CAN_BUS_OFF_INT) ||                       \
     ((CONFIG) == CAN_WAKE_UP_INT) || ((CONFIG) == CAN_ERROR_INT) || ((CONFIG) == CAN_RX_INT) ||   \
     ((CONFIG) == CAN_TX_INT))
...
#define IS_CAN_INT_FLAG(FLAG)                                                                      \
    (((CONFIG) == CAN_RAM_MOVE_DONE_INT_FLAG) || ((CONFIG) == CAN_BUS_OFF_INT_FLAG) ||             \
     ((CONFIG) == CAN_WAKE_UP_INT_FLAG) || ((CONFIG) == CAN_ERROR_INT_FLAG) ||                     \
     ((CONFIG) == CAN_RX_INT_FLAG) || ((CONFIG) == CAN_TX_INT_FLAG))
...
#define IS_CAN_ERROR_STATUS(STATUS)                                                                \
    (((STATUS) == CAN_ERROR_RX) || ((STATUS) == CAN_ERROR_TX) || ((STATUS) == CAN_ERROR_ACK) ||    \
     ((STATUS) == CAN_ERROR_CRC) || ((STATUS) == CAN_ERROR_FORM) ||                                \
     ((STATUS) == CAN_ERROR_BIT1) || ((STATUS) == CAN_ERROR_BIT0))
...
typedef struct
{
    ...
    uint32_t frame_id_mask; /* can frame ID mask, 1 means the ID bit in CAN_RAM_ARB don't care, 0
                               means the bit should match. */
 ...
 *      \arg CAN_RAM_MOVE_DONE_INT: data move from register to CAN IP internal RAM finished
 * interupt. \arg CAN_BUS_OFF_INT: bus off interrupt. \arg CAN_WAKE_UP_INT: wakeup interrupt. \arg
 * CAN_ERROR_INT: error interrupt. \arg CAN_RX_INT: RX interrupt. \arg CAN_TX_INT: TX interrupt.
...
/**
 * \brief  Enable or disable CAN tri-sample function.
 * \param[in] CANx: selected CAN peripheral.
 * \param[in] newState: the state of CAN tri-sample function.
 * \return none.
 */
void CAN_TriSampleCmd(FunctionalState NewState);
